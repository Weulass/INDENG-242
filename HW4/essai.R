...
Ggplot = read.csv("ggplot2questions2016_17.csv", stringsAsFactors=FALSE) #read the data
...
corpus_body = tm_map(corpus_body, removeWords, c("ggplot2", stopwords("english"),"ggplot","r"))
corpus_title=tm_map(corpus_title, removeWords, c("ggplot2", stopwords("english"),"ggplot","r"))
...
#who appears at least in 5% of the body texts of the questions 
#or at least in 1% of the titles
#higher percentage for the body since significatively more words
...
tableTPR <- function(test, pred) { #compute the accuracy
  t = table(test, pred)
  a = t[2,2]/(t[2,1]+t[2,2])
  return(a)
}
tableFPR <- function(test, pred) { #compute the accuracy
  t = table(test, pred)
  a = t[1,2]/(t[1,1]+t[1,2])
  return(a)
}
table(dataTest$Useful, PredictLog > 0.5) #without other informations we use 0.5 as a threshold
tableAccuracy(dataTest$Useful, PredictLog > 0.5)#accuracy of 0.5612
tableTPR(dataTest$Useful,PredictLog>0.5) #TPR=0.510
tableFPR(dataTest$Useful,PredictLog>0.5) #FPR=0.390
...
table(dataTest$Useful, predict.cart)
tableAccuracy(dataTest$Useful, predict.cart)#accuracy of 0.5487 on the test set
tableTPR(dataTest$Useful,predict.cart) #TPR=0.4995
tableFPR(dataTest$Useful,predict.cart) #FPR=0.404

 #Random Forest
set.seed(3421)
dataRF = randomForest(Useful ~ ., data=dataTrain, mtry=21)
PredictRF = predict(dataRF, newdata = dataTest)
table(dataTest$Useful, PredictRF) #confusion matrix
tableAccuracy(dataTest$Useful, PredictRF) #0.5772
tableTPR(dataTest$Useful,PredictRF) #TPR=0.563
tableFPR(dataTest$Useful,PredictRF) #FPR=0.410
...
train.boost
train.boost$results
train.boost$bestTune #n.trees=48 and interaction.depth=12
mod.boost=train.boost$finalModel
dataTest.mm = as.data.frame(model.matrix(Useful ~ . +0, data = dataTest))
predict.boost = predict(mod.boost, newdata = dataTest.mm,n.trees=39, type = "response") #prediction on the test set
table(dataTest$Useful, predict.boost < 0.5) 
tableAccuracy(dataTest$Useful, predict.boost < 0.5) #accuracy=0.5585
...
tableTPR(dataTest$Useful,predict.boost < 0.5) #TPR=0.341
tableFPR(dataTest$Useful,predict.boost < 0.5) #FPR=0.230
...
#precision of models
##Logistic Regression
precision_LogReg=tablePrecision(dataTest$Useful,PredictLog>0.5) #0.5598
boot.ci(LogReg_boot,index=4,type="basic") #CI of precision

##CART
precision=tablePrecision(dataTest$Useful,predict.cart) #0.5598
boot.ci(CART_boot,index=4,type="basic") #CI of precision

##RF
precision=tablePrecision(dataTest$Useful,Predict.RF) #0.5598
boot.ci(RF_boot,index=4,type="basic") #CI of precision

##Boosting
precision=tablePrecision(dataTest$Useful,predict.boost) #0.5598
...
perf_boosting<-function(model){
  ...
  pred = predict(model, newdata = test.mm,n.trees=39, type = "response")
  x=as.numeric(test$Useful[pred==min(pred)]==1) #min here since gbm inverses the probabilities
  y=y+x
  }
  return (y/10000)
}

perf_boosting(mod.boost) #0.64
